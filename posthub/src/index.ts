...
import { PostCreatedListener, PostUpdatedListener } from "./middleware/events-handler/listeners/post-events-listener";
...
const postHubServerStart = async () => {
    ...
    if (!process.env.NATS_CLUSTER_ID) {
        throw new Error(`NATS_CLUSTER_ID ENV is ${process.env.NATS_CLUSTER_ID} and must be defined in the Kubernetes YAML File for this code to run in the Cluster`);
    }

    if (!process.env.NATS_CLIENT_ID) {
        throw new Error(`NATS_CLIENT_ID ENV is ${process.env.NATS_CLIENT_ID} and must be defined in the Kubernetes YAML File for this code to run in the Cluster`);
    }
    ...
    try {
        ...
        await new PostCreatedListener(natsEventClass.client).listen();
        await new PostUpdatedListener(natsEventClass.client).listen();
